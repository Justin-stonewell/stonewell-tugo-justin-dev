...
export default function OptimizedPlanForm(props) {
  ...
  //  console.log(data)
    ...
    data.eligibleStudent = insuredEligibleStudent.length > 0 ? true : false
    ...
    for (var i = 0; i < data.insuredPersons.length; i++) {
        ...
        // if (data.destCountry !== 'CA' && data.insuranceType !== 'CANADIAN') {
        //   data.insuredPersons[i].eligilbeIns = 'WORLDWIDE';
        // } 
        // // else if (data.destCountry !== 'CA' && data.insuredPersons[i].travelType !== 'SS') {
        // //   data.insuredPersons[i].eligilbeIns = 'WORLDWIDE';
        // // } 
        // else if (data.insuranceType === 'CANADIAN') {
        //   data.insuredPersons[i].eligilbeIns = 'CANADIAN';
        // }
        // else if (data.insuranceType !== 'STUDENT' && data.insuranceType !== 'VISITOR' ) {
        //   // Worldwide 
        //   data.insuredPersons[i].eligilbeIns = 'WORLDWIDE';
        // } 
        if (data.destCountry === 'US' && data.insuranceType !== 'CANADIAN') {
          // 목적지가 미국이고 보험 유형이 캐네디언이 아닌 경우
            data.insuredPersons[i].eligilbeIns = 'USA';
        } else if (data.destCountry !== 'CA' && data.insuranceType !== 'CANADIAN') {
            // 목적지가 캐나다가 아니고 보험 유형이 캐네디언이 아닌 경우
            data.insuredPersons[i].eligilbeIns = 'WORLDWIDE';
        } else if (data.insuranceType === 'CANADIAN') {
            // 보험 유형이 캐네디언인 경우
            data.insuredPersons[i].eligilbeIns = 'CANADIAN';
        } else if (data.insuranceType !== 'STUDENT' && data.insuranceType !== 'VISITOR') {
            // 보험 유형이 학생이나 방문자가 아닌 경우, 기본적으로 Worldwide 설정
            data.insuredPersons[i].eligilbeIns = 'WORLDWIDE';
        }
          else
        {
            // 가족중에 학생이 있다면
            // 나이가 69세이하 일 경우 (59세 이하는 알리안츠,투고// 69세 이하는 Travelance Only 제공)
            // inbound
          if (data.eligibleStudent === true && 
              data.insuredPersons[i].age < 70 && 
        ...
        // AllianzPlan is abled to apply if originCountry is Cananda && provinces are 'ON' 'QC', If VendorApplication && Canadian Over 60 always True
          let eligilbeAllianzCANPlan = false
          if(data.sourceFrom !=='V' && (data.originProvince === 'ON' || data.originProvince === 'QC')) {
            eligilbeAllianzCANPlan = true
          }
          else if(data.sourceFrom ==='V' && insuredAge > 59){
            eligilbeAllianzCANPlan = true
          } else if (data.sourceFrom ==='V' && insuredAge < 60 && (data.originProvince === 'ON' || data.originProvince === 'QC')){
            eligilbeAllianzCANPlan = true
          }
        ...
        let isInsuredPerson60to69 = false

        if (data.insuredPersons[i].age < 70 && data.insuredPersons[i].age > 59) {
          isInsuredPerson60to69 = true
        }

        // let isAbleToGetTravelanceStudent = false

        // If Public quote or Stonewell, Show Travelance Student
        // if (data.vendorID === "VEN0000010") {
        //   isAbleToGetTravelanceStudent = true
        ...
        if (data.insuredPersons[i].eligilbeIns === 'STUDENT') 
        {
                                                          ...
                                                          && (isAbleToGetAllianz === true 
                                                              ?  ( data.insuredPersons.filter(f=> f.travelType ==='SS').length === 0 &&
                                                                    data.insuredPersons.filter(f => f.travelType === 'PW' && f.yearDateAfterGraduated >= f.tripEndDate).length>0
                                                                      ...
                                                                      : ins.compnay_name !== '' )
                                                              :  ins.compnay_name !== 'Allianz')
                                                          // If Vendor Application, No Travelance Student
                                                          && (data.sourceFrom ==='V' ? ins.compnay_name !== 'Travelance': ins.compnay_name !== '')
                                                          // Travelance for 60-69 Student Companion 
                                                          && (isInsuredPerson60to69 === true ? ins.compnay_name === 'Travelance' : ins.compnay_name !== '')
                                                          // TuGo and Travelance for 60-69 student
                                                          // && (data.insuredPersons[i].age < 70 && data.insuredPersons[i].age > 59 && data.insuredPersons[i].travelType === 'SS' ? ins.compnay_name === 'Tugo': ins.compnay_name !== '')
          ...
          eligilbePlans = insurances.filter(ins => (ins.insured_type === insuredEligilbeIns)
                                                        ...
                                                        && (data.destProvince === 'QC'
                                                            ...
                                                            : ins.compnay_name !== '')
                                                        // If Allianz, show only $25000 to $100000
                                                        && (ins.compnay_name === 'Allianz'
                                                        ? (parseInt(ins.price_code) >= 25000 && parseInt(ins.price_code) <= 100000)
                                                        : (parseInt(ins.price_code) > 0))    
                                                        )
        ...
        // WorldWide Plan
        else if (data.insuredPersons[i].eligilbeIns === 'WORLDWIDE') {
          eligilbePlans = insurances.filter(ins => ins.compnay_name === 'IMG'
            && (ins.trip_length_min <= tripLength && ins.trip_length_max >= tripLength)
            && (ins.age_min <= insuredAge && ins.age_max >= insuredAge)
          );
        ...
        // USA Plan
        else if (data.insuredPersons[i].eligilbeIns === 'USA') {
          eligilbePlans = insurances.filter(ins => ins.plan_id === 'PLVSMED005'
            && (ins.trip_length_min <= tripLength && ins.trip_length_max >= tripLength)
            && (ins.age_min <= insuredAge && ins.age_max >= insuredAge)
          );
        }
        ...
        let insurancePlans = uniqueListInsCompany.map( c => { 
...
// 고유한 insurancePlans 생성 로직
eligilbePlans.forEach(d => {
  if (!d.plan_id) return; // plan_id가 없는 경우 생략

  let plan = insurancePlans.find(g => g.compnayName === d.compnay_name && g.coverages.some(c => c.plan_id === d.plan_id));

  if (!plan) {
    plan = {
      compnayName: d.compnay_name,
      coverages: [],
      selectedPlanName: '',
      selectedPlanNameKr: '',
      selectedCoverage: '',
      multi_trip_rate: null,
      calculatedDeductAmount: 0,
      calculatedInsuranceAmount: 0,
      medicalQuestion: {
        quesAnswer: [],
  ...
  plan.coverages.push(d);

  const multiTripCoverage = plan.coverages.find(c => c.trip_type === 'MULTI');
  ...
  if (plan.selectedCoverage === '') {
    ...
    plan.selectedCoverage = selectedCoverage;
  }
...
// coverages가 빈 배열인 insurancePlans 제거
insurancePlans = insurancePlans.filter(plan => plan.coverages.length > 0);
