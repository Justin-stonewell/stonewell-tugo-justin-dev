...
export default function Application({ vendorAccessCode, insuraceCompany, insuraceType, applyType, sourceChnnel, renewal, insured, contact }) {
  ...
  const getQuotesAllianz = useCallback(() => {
    dispatch(quoteAllianz({data: formData}))
  }, [dispatch, formData]);
  ...
  useEffect(() => {
    ...
    //If Allianz Canadian, call api
    if(insuraceCompany === 'Allianz' && activeStep === 2 ){
      getQuotesAllianz();
    }
  
  }, [getResourceData, getQuotesAllianz, insuraceCompany, isLoaded]);
          ...
          if (values.insuredPersons
                .filter(i=> i.birthDate && i.ageDays > 14 && i.relationship && i.firstName && i.lastName && i.gender && i.travelType &&
                  ...
                  if(values.application.applicationCompany==='Allianz'){
                        getQuotesAllianz();
                        // quoteAllianzResult에 결과가 성공적으로 저장되었는지 확인합니다.
                          ...
                          if (medicalProductNoDeductible) {
                            ...
                            values.insuredPersons.forEach((person, index) => {
                              ...
                              // if (premiumValue !== undefined) {
                                // values.insuredPersons[index].selectedPlan.calculatedInsuranceAmount = premiumValue;
                                // values.insuredPersons[index].selectedPlan.insuranceAmount = premiumValue;
                                ...
                                // setFieldValue({ ...values, insuredPersons: newInsuredPersons });
