                ...
                <div style={{ display: 'none' }}>
                    ...
                    <CustomButton
                        ...
                        onClick={() => {
                            ...
                            values.insuredPersons.every((person, i) => {
                                if (
                                    (person.selectedPlan && person.selectedPlan.length !== 0) &&
                                    (
                                        (person.selectedPlan.compnayName === 'Allianz' && person.age > 64) ||
                                        (person.selectedPlan.compnayName === 'Tugo' && person.age > 59)
                                    ) &&
                                    (
                                        (person.selectedPlan.medicalQuestion.answered === false) ||
                                        (person.selectedPlan.medicalQuestion.answered === true && person.selectedPlan.medicalQuestion.chargeRate === '0')
                                    ...
                                    if (person.selectedPlan.medicalQuestion.answered === false) {
                                        ...
                                        setErrorMsg(`We are sorry, ${
                                            person.firstName ? person.firstName : i === 0 ? 'Primary' : `Family ${i}`
                                        } is not eligible to purchase this plan, please remove ${i === 0 ? 'Primary' : `Family ${i}`} for purchase or contact us.`);
                                    }
                    ...
                    <Grid container className={classes.row_input}>
                        ...
                        {/* <div style={{ display: (values.insuredPersons[index].insurancePlans.length > 0)
                            ...
                            {values.tripStartDate && values.tripEndDate &&
                                ...
                                values.tripPeriod > 0 && values.tripPeriod < 731 &&
        ...
        <>
            <Grid container spacing={2} ref={scrollRef} style={{ paddingLeft: width > 1400 ? '22vh' : '0', marginTop: width > 1400 ? '-180px' : '0' }}>
                {process.env.NODE_ENV !== 'production' && (
                    <div style={{ background:'#fff7d6', border:'1px solid #ffcc80', padding:'6px 10px', margin:'6px 0', fontSize:12 }}>
                        endpoint={QUOTE_ENDPOINT}, useFunctions={String(!Boolean(process.env.REACT_APP_API_URL))}, useTugoApiPricing={(window.__QUOTE_HEALTH__ && window.__QUOTE_HEALTH__.useTugoApiPricing) || 'n/a'}
                        <div style={{ marginTop: 6 }}>
                            <button type="button" onClick={onDebugQuote} style={{ fontSize:12 }}>🔎 Force TuGo Quote (debug)</button>
                        </div>
                    </div>
                )}
                    ...
                    <Grid item container xs={12} style={{ marginBottom:'2vh' }}>
                        ...
                        <Grid item xs={12} sm={12} md={12} >
                            <Grid item xs container direction="column" spacing={2}>
                                ...
                                <Grid item xs>
                                    {(values.insuredNumber > 0 && values.eligilbeAgrement === true)
                ...
                {values.insuredGroupType === 'Family' &&
                    ...
                    <Grid item container xs={12} sm={12} md={12} style={{ fontSize:'12px', color:'#555'}}>
                        {/* <Text tid={'TravelApplication.Product.Exclusion'}/> */}
                        <Grid item xs={12} sm={12} md={12} lg={10}>
                        * The quoted premium rate is based on the trip details and other information provided by you. Please note that the quoted rates may change as a result of any updates to the information.
                        </Grid> 
                    </Grid>
