...
export default function ApplicationPage() {
  const [formState, setFormState] = useState({ insuredPersons: [] })
  ...
  async function handleGetQuote(e) {
    ...
    try {
      // 간단 프런트 유효성
      if (!formState.residenceCountry || !formState.startDate || !formState.endDate || !Array.isArray(formState.insuredPersons) || formState.insuredPersons.length < 1) {
  ...
  async function handleApply(e) {
    ...
    setError(null); setLoading(true)
    try {
      const applicationPayload = { /* 기존 신청 페이로드: 폼 상태에서 필요 필드 조합 */ }
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <form onSubmit={handleGetQuote}>
        ...
        <div>
          <label>주(선택)</label>
          <input value={formState.province || ''} onChange={(e) => updateField('province', e.target.value)} />
        </div>
        <div>
          <label>시작일</label>
        ...
        <div>
          <label>가족 플랜</label>
          <input type="checkbox" checked={!!formState.isFamilyPlan} onChange={(e) => updateField('isFamilyPlan', e.target.checked)} />
        </div>
